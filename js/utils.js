    ...
    loadLightbox: ele => {
      ...
      if (service === 'fancybox') {
        ...
        if (!window.fancyboxRun) {
          ...
          if (!fancybox) {
            const fancyboxScript = document.querySelector('script[src*="fancybox"]')
            if (fancyboxScript && !fancyboxScript.dataset.retryBind) {
              fancyboxScript.dataset.retryBind = 'true'
              fancyboxScript.addEventListener('load', () => {
                window.fancyboxRun = false
                btf.loadLightbox(ele)
              }, { once: true })
            }
            return
          }
